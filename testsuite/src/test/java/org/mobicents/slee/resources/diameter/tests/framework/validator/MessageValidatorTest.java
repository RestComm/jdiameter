...
public class MessageValidatorTest {
  ...
  public void testBasicOperations()
  {
    ...
    AccountingAnswerImpl answer = (AccountingAnswerImpl) baseFactory.createAccountingAnswer(baseFactory.createAccountingRequest(new DiameterAvpImpl[]{new DiameterAvpImpl(263, 0L, 0, 1, "xxx".getBytes(), DiameterAvpType.UTF8_STRING)}));
    ...
    if(answer.hasSessionId()) {
      assertFalse("We should not allow to add more SessionIds than one, operation indicates that it could be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer.getHeader()
          .getApplicationId(), false, set, 263, 0L));
    }
    else {
      assertTrue("We should allow to add ONE (1) Session-Id AVP, operation indicates that it could not be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer.getHeader()
          .getApplicationId(), false, set, 263, 0L));		  
    }
    ...
    if(answer.hasOriginHost()) {
      assertFalse("We should allow to add more Origin-Host than zero, operation indicates that it could not be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer
          .getHeader().getApplicationId(), false, set, 264, 0L));
    }
    else {
      assertTrue("We should not allow to add more than one Origin-Host, operation indicates that it could be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer
          .getHeader().getApplicationId(), false, set, 264, 0L));
    }
    ...
    assertTrue("We should allow to add more Event-Timestamp than zero, operation indicates that it could not be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer
        .getHeader().getApplicationId(), false, set, 55, 0L));

    set.addAvp(55, 55L, 0L, true, false);
    assertFalse("We should not allow to add more Event-Timestamp than one, operation indicates that it could be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer
        .getHeader().getApplicationId(), false, set, 55, 0L));
    ...
    assertTrue("We should allow to add more Proxy-Info than zero, operation indicates that it could not note be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer
    ...
    assertFalse("We should not  allow to add more Auth-Application-Id than zero, operation indicates that it could be done.", this.instance.isCountValidForMultiplicity(AccountingAnswer.commandCode, answer
