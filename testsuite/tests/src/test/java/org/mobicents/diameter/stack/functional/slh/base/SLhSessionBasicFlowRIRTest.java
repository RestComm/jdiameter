...
public class SLhSessionBasicFlowRIRTest {
   ...
   * @param serverCount
   */
  public SLhSessionBasicFlowRIRTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void testRoutingInfo() throws Exception {
    try {
      // pain of parameter tests :) ?
      clientNode.sendLCSRoutingInfo();
      waitForMessage();

      serverNode1.sendRoutingInfo();
      waitForMessage();
    }
    catch (Exception e) {
      e.printStackTrace();
    ...
    if (!serverNode1.isReceiveRoutingInfo()) {
      StringBuilder sb = new StringBuilder("Did not receive Routing Info! ");
      sb.append("Server ER:\n").append(serverNode1.createErrorReport(this.serverNode1.getErrors()));

      fail(sb.toString());
    }
    if (!clientNode.isReceiveRoutingInfo()) {
      StringBuilder sb = new StringBuilder("Did not receive Routing Info! ");
  ...
  public static Collection<Object[]> data() {
    ...
    Class<SLhSessionBasicFlowRIRTest> t = SLhSessionBasicFlowRIRTest.class;
