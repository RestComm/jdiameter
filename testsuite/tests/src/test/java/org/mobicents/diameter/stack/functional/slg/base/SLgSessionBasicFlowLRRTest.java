...
public class SLgSessionBasicFlowLRRTest {
  // TODO: add test on replicated nodes ?
  private ClientLRR clientNode;
  private ServerLRR serverNode1;
   ...
   * @param serverCount
   */
  public SLgSessionBasicFlowLRRTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void setUp() throws Exception {
    try {
      ...
      this.serverNode1 = new ServerLRR();
  ...
  @Test
  public void testLocationReport() throws Exception {
    try {
      // pain of parameter tests :) ?
      serverNode1.sendLocationReport();
      waitForMessage();

      clientNode.sendLocationReport();
      waitForMessage();
    }
    catch (Exception e) {
      e.printStackTrace();
    ...
    if (!clientNode.isReceiveLocationReport()) {
      StringBuilder sb = new StringBuilder("Did not receive Location Report Requset! ");
      sb.append("Client ER:\n").append(clientNode.createErrorReport(this.clientNode.getErrors()));

      fail(sb.toString());
    }
    if (!serverNode1.isReceiveLocationReport()) {
      StringBuilder sb = new StringBuilder("Did not receive Location Report Answer! ");
  ...
  public static Collection<Object[]> data() {
    ...
    Class<SLgSessionBasicFlowLRRTest> t = SLgSessionBasicFlowLRRTest.class;
