...
public class SLgSessionBasicFlowPLRTest {
  private ClientPLR clientNode;
  private ServerPLR serverNode1;
   ...
   * @param serverCount
   */
  public SLgSessionBasicFlowPLRTest(String clientConfigUrl, String serverNode1ConfigURL) throws Exception {
    super();
  ...
  public void setUp() throws Exception {
    try {
      ...
      this.serverNode1 = new ServerPLR();
  ...
  @Test
  public void testProvideLocationReqRes() throws Exception {
    try {
      // pain of parameter tests :) ?
      clientNode.sendLocationRequest();
      waitForMessage();

      serverNode1.sendLocationAnswer();
      waitForMessage();
    }
    catch (Exception e) {
      e.printStackTrace();
    ...
    if (!serverNode1.isReceiveLocationRequest()) {
      StringBuilder sb = new StringBuilder("Did not receive Provide Location request! ");
      sb.append("Server ER:\n").append(serverNode1.createErrorReport(this.serverNode1.getErrors()));

      fail(sb.toString());
    }
    if (!clientNode.isReceiveLocationAnswer()) {
      StringBuilder sb = new StringBuilder("Did not receive Provide Location Answer! ");
  ...
  public static Collection<Object[]> data() {
    ...
    Class<SLgSessionBasicFlowPLRTest> t = SLgSessionBasicFlowPLRTest.class;
