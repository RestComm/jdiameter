...
public class DiameterMessageValidator {
  ...
  //								}
  //							} catch (Exception e) {
  //								log.warn("Failed to parse AVP values.", e);
  //							}
  //						}
  //
  //					}
  //				} catch (Exception e) {
  //          log.warn("Failed to parse Message values.", e);
  //				}
  //			}
  //		} catch (Exception e) {
  //      log.warn("Failed to parse InputStream.", e);
  //		}
  //
  //	}
  ...
  public void parseConfiguration(InputStream is, boolean reset)
  {
    ...
    try {
      ...
      log.info("AVP Validator :: Loaded in {}ms == Vendors[{}] Commands[{}] Types[{}] AVPs[{}]", 
              ...
              for (int commandAvpIndex = 0; commandAvpIndex < commandAvpsList.getLength(); commandAvpIndex++) {
                if (commandAvpsList.item(commandAvpIndex).getNodeType() == Node.ELEMENT_NODE) {
                  ...
                  if (!commandAvpElement.hasAttribute("name")) {
                    log.error("Command defines avp without name! Command: {}, Code: {}, ApplicationID: {}", new Object[]{msg.getName(), msg.getCommandCode(), msg.getApplicationId()});
                    continue;
                  }
                  else {
  ...
  public Map<VAvpRepresentation, VAvpRepresentation> parseAvps(Document doc, Map<String, String> vendorMap) {
    ...
    HashMap<VAvpRepresentation, VAvpRepresentation> avpMap = new HashMap<VAvpRepresentation, VAvpRepresentation>();
    for (int s = 0; s < avpNodes.getLength(); s++) {

      Node avpNode = avpNodes.item(s);
      if(avpNode.getParentNode().getNodeName().equals("command")) {
        continue;
      }
      /*
       * <!ELEMENT avp ((type | grouped), (enum*))> <!ATTLIST avp name ID
       * #REQUIRED description CDATA #IMPLIED code CDATA #REQUIRED
       * may-encrypt (yes | no) "yes" mandatory (must | may | mustnot |
       * shouldnot) "may" protected (must | may | mustnot | shouldnot)
       * "may" vendor-bit (must | may | mustnot | shouldnot) "mustnot"
       * vendor-id IDREF #IMPLIED constrained (true | false) "false" >
       */
      if (avpNode.getNodeType() == Node.ELEMENT_NODE) {
        ...
        try {
          ...
          if (log.isErrorEnabled()) {
            log.error("Failed Parsing AVP: Name[" + avpName + "] Description[" + avpDescription + "] Code[" + avpCode + "] May-Encrypt[" + avpMayEncrypt + "] Mandatory[" + avpMandatory
                + "] Protected [" + avpProtected + "] Vendor-Bit [" + avpVendorBit + "] Vendor-Id [" + avpVendorId + "] Constrained[" + avpConstrained + "] Type [" + avpType + "]", e);
          }
        }
      }
    }
  ...
  public void resolveWeakGroupedChildren( Map<VAvpRepresentation, VAvpRepresentation> avpMap, Map<String, VAvpRepresentation> nameToCodeMap) {
    //FIXME: we have maximum 50 runs, this does not take much time, limits number of iterations over collection to fill all data.
  ...
  public void validate(Message msg) throws JAvpNotAllowedException {
    ...
    if (rep == null) {
      ...
      log.warn("Validation could not be performed for Command. Code={}, Application-Id={}, Req={}", 
