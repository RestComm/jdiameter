...
/**
 * Basic implementation of SLhServerSession - can be one time - for RIR
...
public class SLhServerSessionImpl extends SLhSession implements ServerSLhSession, EventListener<Request, Answer>, NetworkReqListener {
  ...
  private Logger logger = LoggerFactory.getLogger(SLhServerSessionImpl.class);
  ...
  protected transient ServerSLhSessionListener listener;

  protected ISLhServerSessionData sessionData;
  protected long appId;
  public SLhServerSessionImpl(ISLhServerSessionData sessionData, ISLhMessageFactory fct, ISessionFactory sf, ServerSLhSessionListener lst) {
  ...
  @Override
  public void sendLCSRoutingInfoAnswer(LCSRoutingInfoAnswer answer) throws InternalException, IllegalDiameterStateException, RouteException, OverloadException {
	  send(Event.Type.SEND_LCS_ROUTING_INFO_ANSWER, null, answer);
  }
  ...
  public boolean handleEvent(StateEvent event) throws InternalException, OverloadException {
    try {
      ...
      case RECEIVE_LCS_ROUTING_INFO_REQUEST:
        listener.doLCSRoutingInfoRequestEvent(this, (LCSRoutingInfoRequest) localEvent.getRequest());
        break;
      case SEND_LCS_ROUTING_INFO_ANSWER:
  ...
  public boolean equals(Object obj) {
    ...
    if (getClass() != obj.getClass())
      return false;
    SLhServerSessionImpl other = (SLhServerSessionImpl) obj;
      ...
      try {
        if (request.getApplicationId() == appId) {
          if(request.getCommandCode() == LCSRoutingInfoRequest.code) {
            handleEvent(new Event(Event.Type.RECEIVE_LCS_ROUTING_INFO_REQUEST, factory.createLCSRoutingInfoRequest(request), null));
          }
          else {
  ...
  private class AnswerDelivery implements Runnable {
    ...
    public void run() {
      try {
        sendAndStateLock.lock();
        if (request.getApplicationId() == appId) {
        	// We do not except any Response on Server session
